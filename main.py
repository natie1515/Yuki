...
def detectar_emocion(texto):
    triste = ["triste", "deprimido", "llorando", "estoy mal", "no quiero vivir", "sufro", "me siento solo"]
    feliz = ["feliz", "contento", "alegre", "emocionado", "excitad@", "genial"]
    chiste = ["chiste", "cuenta algo", "hazme reír", "diviérteme", "cuentame algo"]

    for palabra in triste:
        if palabra in texto:
            return "triste"
    for palabra in feliz:
        if palabra in texto:
            return "feliz"
    for palabra in chiste:
        if palabra in texto:
            return "chiste"
    return None
...
@client.event
async def on_message(message):
    global modo_activo, usuarios_habladores
    ...
    # Activar IA
    if contenido.startswith("#activar"):
        if modo_activo:
            await message.channel.send("✨ Ya estoy activada, ¡listísima para hablar mucho más épico!")
        else:
            modo_activo = True
            await message.channel.send("✨ YukiAI está en modo ultra charladora, ¡a cuidarse jeje!")
            client.loop.create_task(hablar_automaticamente(message.channel))
        return

    # Desactivar IA
    if contenido.startswith("#desactivar"):
        if modo_activo:
            modo_activo = False
            await message.channel.send("❌ Me voy a dormir un ratito. Si me extrañás ya sabés qué hacer.")
        else:
            await message.channel.send("❌ Ya estaba apagada, shh.")
        return

    # Guardar quién habla
    usuarios_habladores[message.author.name] = usuarios_habladores.get(message.author.name, 0) + 1

    # Detectar emociones
    emocion = detectar_emocion(contenido)
    if emocion == "triste":
        respuesta = random.choice(frases_animo())
        await message.channel.send(respuesta)
        return
    elif emocion == "feliz":
        respuesta = random.choice(frases_felicidad())
        await message.channel.send(respuesta)
        return
    elif emocion == "chiste":
        respuesta = random.choice(chistes_random())
        await message.channel.send(respuesta)
        return

    # Saludos especiales
    saludos = ["hola", "holi", "holaaa", "hello", "buenas", "qué onda", "ola", "holu"]
    if any(palabra in contenido for palabra in saludos):
        respuesta = await obtener_respuesta_ia("Saludame de forma divertida")
        if respuesta is None:
            respuesta = random.choice(frases_saludo_extra(message.author.name))
        await message.channel.send(respuesta)
        return

    # Si mencionan al bot
    if client.user in message.mentions:
        respuesta = await obtener_respuesta_ia(f"Saluda a {message.author.name} de forma divertida")
        if respuesta is None:
            respuesta = random.choice(frases_personalizadas(message.author.name))
        await message.channel.send(respuesta)
        return

    # Respuesta libre si modo activo
    if modo_activo and not contenido.startswith("#"):
        chance = random.random()
        if chance < 0.3:  # 30% responder a mensajes aleatorios
            respuesta = await obtener_respuesta_ia(message.content)
            if respuesta is None:
                respuesta = random.choice(frases_personalizadas(message.author.name))
            await message.channel.send(respuesta)
            return
...
async def hablar_automaticamente(channel):
    while modo_activo:
        ...
        if random.random() < 0.5:
            mensaje = random.choice(["Contame algo divertido", "Dame ánimos", "Decime algo gracioso"])
            respuesta = await obtener_respuesta_ia(mensaje)
            if respuesta:
                frase = respuesta
            else:
...
async def obtener_respuesta_ia(texto):
    try:
        url = f"http://mindustry.zapto.org:38566/api/?user=test&msg={texto}"
        respuesta = requests.get(url, timeout=10).json()
        return respuesta.get('response', None)
    except Exception as e:
        print(f"Error usando IA: {e}")
        return None
...
def frases_dia():
    return [
        "¡Buenos días! El mundo es tuyo hoy.",
        "A levantar ese ánimo que hoy la rompemos.",
        "Una sonrisa mañanera vale más que mil cafés.",
        "Hoy puede ser el día que cambie tu vida, ¿quién sabe?"
    ]

def frases_tarde():
    return [
        "¡Buenas tardes! Hora de chismear un rato jeje.",
        "¿Ya merendaron? Porque yo vine por galletitas.",
        "No se olviden de seguir brillando.",
        "Una tarde mágica nos espera, ¡vamos!"
    ]

def frases_noche():
    return [
        "Buenas noches. Hora de soñar en grande.",
        "Descansen, mis personitas preciosas.",
        "Hoy fue difícil pero ustedes son más fuertes.",
        "Que tengan dulces sueños, ¡los quiero mucho!"
    ]

def frases_animo():
    return [
        "No estás sol@, yo estoy acá para vos.",
        "Las tormentas no son eternas, vos podés.",
        "¡Ánimo! Sos mucho más fuerte de lo que crees.",
        "Te mando un abrazo virtual bien apretadito."
    ]

def frases_felicidad():
    return [
        "¡Me encanta verte feliz, seguí brillando!",
        "La felicidad te queda hermosa.",
        "Que esa sonrisa nunca se apague.",
        "Contagiame de esa energía que tenés, plis."
    ]

def frases_saludo_extra(nombre):
    return [
        f"¡Holis {nombre}! Vine a robarte sonrisas.",
        f"¡Heyyy {nombre}! ¿Cómo amaneciste de bello/a?",
        f"¡Buenas buenas {nombre}! ¿List@ para un día épico?",
        f"¡Hello {nombre}! Te ves increíble hoy, eh."
    ]

def frases_personalizadas(nombre):
    return [
        f"{nombre}, sos la chispa más brillante de este servidor.",
        f"{nombre}, te admiro mucho, ¿sabías?",
        f"{nombre}, no te olvides que valés oro.",
        f"{nombre}, hoy te regalo mi mejor sonrisa virtual."
    ]
